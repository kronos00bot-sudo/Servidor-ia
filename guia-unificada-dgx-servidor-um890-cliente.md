...
23. [Conectar el UM890 al DGX Spark](#23-conectar-el-um890-al-dgx-spark)
  - [23.4 WhatsApp Skill — UM890 + DGX](#234-whatsapp-skill--um890--dgx)
...
## 23.4 WhatsApp Skill — UM890 + DGX

Implementación actual de producción: **completa y operativa** en modo distribuido UM890 + DGX.

- UM890: orquestación, visión local, chat rápido y fallback local.
- DGX: Whisper remoto, razonamiento pesado, traducción larga y reporte final.

### 23.4.1 Alcance real del proyecto

Este skill procesa exportes de WhatsApp de extremo a extremo:

- Parseo del export `_chat.txt` (formato real en español `dd/mm/yy, hh:mm - Sender: text`)
- Clasificación de tipos: texto, audio, video, imagen, documento, omitido
- STT de audio/video con Whisper remoto DGX
- Visión de imágenes con fallback automático UM890 -> DGX
- Procesamiento de documentos PDF
- Interpretación de conversación completa
- Traducción y generación de reporte final en español
- Webhook de WhatsApp Business API (Meta) con validación HMAC

### 23.4.2 Estructura del código (workspace OpenClaw)

Ruta base:

```bash
/home/mloco/Escritorio/Servidor-ia/agente_workspace/skills/whatsapp
```
...
```text
skills/whatsapp/
├── whatsapp_skill.py                 # entrypoint CLI principal
├── core/
│   ├── parser.py                     # parse export WhatsApp
│   ├── transcriber.py                # STT audio/video
│   ├── image_processor.py            # visión + fallback DGX
│   ├── video_processor.py            # frames + audio de video
│   ├── document_processor.py         # resumen de documentos
│   ├── interpreter.py                # análisis conversacional
│   ├── translator.py                 # traducción + reporte final
│   └── pipeline.py                   # orquestador de fases
├── routing/
│   ├── task_router.py                # política de ruteo UM890/DGX
│   └── remote_client.py              # cliente de inferencia remota
├── utils/
│   ├── config.py                     # configuración/timeout/env
│   ├── http_client.py                # HTTP robusto con retry
│   ├── infrastructure.py             # health checks
│   ├── prepare_dataset.py            # preparación de dataset
│   └── batch_run.py                  # ejecución por lotes + resume
├── webhook/
│   ├── endpoint.py                   # Flask webhook Meta
│   └── client.py                     # cliente Graph API
├── tests/                            # tests unitarios/integración
├── .env.example
├── README.md
└── SKILL.md                          # metadata de integración OpenClaw
```

### 23.4.3 Política de ruteo UM890 + DGX

| Tarea | Preferido | Fallback |
|---|---|---|
| Transcripción audio/video | DGX Whisper (`DGX_WHISPER_URL`) | reintento con timeout extendido |
| Visión imagen | UM890 `gemma4-es` | DGX `gemma4:26b` |
| Chat corto | UM890 `qwen35-es` | UM890 alternativo |
| Razonamiento complejo | DGX `nemotron-3-super:120b` / `gpt-oss:120b` | modelo DGX alternativo |
| Traducción larga/reporte | DGX 120B | modelo DGX alternativo |
| Documento PDF | DGX 120B | modelo DGX alternativo |

### 23.4.4 Configuración de entorno (mínima)

Archivo recomendado: `PROJECT_DIR/.env` (prioridad sobre cwd).

Variables clave:

- `PROJECT_DIR=/home/mloco/Escritorio/Servidor-ia/whatsapp-agent`
- `MACHINE_TYPE=um890`
- `LOCAL_OLLAMA_URL=http://127.0.0.1:11434`
- `DGX_OLLAMA_URL=http://100.64.129.87:11434`
- `DGX_WHISPER_URL=http://100.64.129.87:8765/inference`
- `VISION_MODEL=gemma4-es`
- `DGX_VISION_MODEL=gemma4:26b`

Timeouts recomendados de producción:

- `TIMEOUT_VISION=180`
- `TIMEOUT_VISION_FALLBACK=240`
- `TIMEOUT_WHISPER=300`
- `TIMEOUT_DOCUMENT=180`
- `TIMEOUT_REASONING=180`
- `TIMEOUT_TRANSLATION=600`

### 23.4.5 Instalación y ejecución
...
PROJECT_DIR=/home/mloco/Escritorio/Servidor-ia/whatsapp-agent \
  ../agente_workspace/skills/whatsapp/.venv/bin/python \
  -m skills.whatsapp.whatsapp_skill --chat /ruta/_chat.txt
```

### 23.4.6 Operación por lotes (dataset grande)

Runner de lotes:

```bash
python -m skills.whatsapp.utils.batch_run --type <audio|image|video|document|media|llm|all> --resume
```

Tipos:

- `audio`: transcripción de audio/video
- `image`: visión de imágenes
- `video`: procesamiento específico de video
- `document`: extracción/resumen de documentos
- `media`: audio+image+video+document (sin LLM)
- `llm`: interpretación + traducción + reporte final
- `all`: todo

`--resume` reusa `data/conversations/parsed.json` y evita reprocesar.

### 23.4.7 Uso a través del agente OpenClaw

Estado de integración:

- Skill con metadata declarada en `skills/whatsapp/SKILL.md`
- Flujo invocable por prompts de agente

Uso recomendado:

1. Reinicia sesión del agente para recargar skills.
2. En chat del agente, pide la acción concreta (procesar, reanudar, webhook, etc.).

Ejemplos de prompts válidos:

- "Usa el skill whatsapp y procesa `/home/mloco/Escritorio/Servidor-ia/whatsapp-agent/media/input/_chat.txt`"
- "Reanuda whatsapp con `PROJECT_DIR=/home/mloco/Escritorio/Servidor-ia/whatsapp-agent`"
- "Ejecuta `--type media --resume` y luego `--type llm --resume`"

### 23.4.8 Webhook Meta (WhatsApp Business)
...
PROJECT_DIR=/home/mloco/Escritorio/Servidor-ia/whatsapp-agent \
  ../agente_workspace/skills/whatsapp/.venv/bin/python \
  -m skills.whatsapp.webhook.endpoint
```

Incluye:

- Verificación GET `/webhook`
- Firma HMAC SHA-256 en POST
- Rate-limit básico por remitente
- Envío de respuestas vía Graph API

### 23.4.9 Estado actual de implementación

Resultado operativo del proyecto:

- Media completada (audio/video/documentos) en dataset real
- Reporte final generado (`output/conversation_es.md` y `output/conversation_es.json`)
- Suite de tests en verde (37 passing)

Nota operativa:

- Quedaron 2 imágenes con resultado vacío histórico (IDs 125 y 156). No bloquean el flujo E2E; pueden reintentarse con fallback DGX.

### 23.4.10 Estándar actual de salida (core)

Desde esta versión, quedó definido como comportamiento estándar del pipeline:

- `core/translator.py` fuerza bloque multimedia en español en la salida final.
- Si la traducción larga omite multimedia, el sistema reinyecta eventos de `AUDIO/VIDEO/IMAGE/DOCUMENT`.
- Si falla traducción por lote, aplica fallback por línea para evitar dejar transcripciones en inglés.
- Se eliminan líneas obsoletas tipo `[FILE - not processed]` cuando ya existe resultado real en caché.

Resultado esperado tras `--type llm --resume`:

- `output/conversation_es.md` con sección `## Conversación Traducida` y bloque `[EVENTOS MULTIMEDIA EN ESPAÑOL]`.
- `output/conversation_es.json` con `translated_conversation` consistente en español.
...
python3 - <<'PY'
import json
p='/home/mloco/Escritorio/Servidor-ia/whatsapp-agent/output/conversation_es.json'
with open(p, encoding='utf-8') as f:
  d=json.load(f)
text=d.get('translated_conversation','')
print('AUDIO', text.count('🎤'))
print('VIDEO', text.count('🎥'))
print('IMAGE', text.count('🖼️'))
print('DOCUMENT', text.count('📄'))
PY
```

### 23.4.11 Troubleshooting rápido

| Problema | Causa probable | Solución |
|---|---|---|
| `FileNotFoundError: Chat file not found` | Ruta `--chat` inválida o `PROJECT_DIR` incorrecto | Verificar `PROJECT_DIR` y ruta real del `_chat.txt` |
| `ModuleNotFoundError: No module named 'skills'` | Ejecución fuera de `agente_workspace` | Ejecutar desde workspace o ajustar `PYTHONPATH` |
| DGX Ollama no responde | Falta tailnet o URL errónea | `tailscale status` + `curl http://100.64.129.87:11434` |
| Whisper DGX no responde | Servicio Whisper caído/puerto no expuesto | Revisar proceso en DGX y puerto `8765` |
| Visión local tarda demasiado | Modelo local lento/saturado | Mantener timeout `180s` y fallback DGX habilitado |
| LLM final tarda mucho | Conversación muy larga en 120B | Ejecutar `--type llm --resume` y esperar; revisar output al final |
| Multimedia aparece en inglés en el reporte | Salida vieja o corrida previa sin estándar nuevo | Re-ejecutar `--type llm --resume` y validar `output/conversation_es.{md,json}` |
| Skill no se invoca desde el agente | Sesión no recargó metadata | Reiniciar sesión del agente OpenClaw |
