...
def extract_data_from_pdf(pdf_path, campo_code):
    ...
    try:
        with pdfplumber.open(pdf_path) as pdf:
            ...
            # For specific text labels
            else:
                label_variants = [campo_code.lower()]
                # Add common variants for better matching
                ...
                if 'base liquidable general' in campo_code.lower() or campo_code == '(0505)':
                    label_variants.extend([
                        'base liquidable general',
                        'base general',
                        '0505',
                    ])
                if 'base liquidable del ahorro' in campo_code.lower() or campo_code == '(0510)':
                    label_variants.extend([
...
def main():
    ...
    for year_dir in sorted(os.listdir(base_path)):
        ...
        for person_dir in sorted(os.listdir(year_path)):
            ...
            if renta_pdf:
                print(f"  [{person_dir} {year}] Procesando Renta: {os.path.basename(renta_pdf)}")
            ...
            if patrimonio_pdf:
                print(f"  [{person_dir} {year}] Procesando Patrimonio: {os.path.basename(patrimonio_pdf)}")
