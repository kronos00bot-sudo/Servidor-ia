    ...
    try:
        with pdfplumber.open(pdf_path) as pdf:
            ...
            if campo_code.startswith('(') and campo_code.endswith(')'):
                # Look for the pattern: campo code followed by the value (same or next lines)
            ...
            # For specific text labels like "Total de bienes y derechos no exentos" or "Cuota a ingresar"
                ...
                # Normalize the label for flexible matching
                label_variants = [
                    campo_code.lower(),
                    campo_code.lower().replace(' ', ''),
                    campo_code.lower().replace(' ', '.'),
                ]
                # Add common variants
                if 'total de bienes y derechos no exentos' in campo_code.lower():
                ...
                if 'cuota a ingresar' in campo_code.lower():
                    label_variants.extend([
                        'cuota a ingresar',
                        'cuota ingresar',
                        'cuota',
...
def find_matching_file(directory, person_name, year, doc_type):
    """
    Find a PDF file in directory that matches:
    - Contains the person's name (as substring, case-insensitive)
    - Contains doc_type (e.g., 'Just. presentación Renta' or 'Just. presentación I. Patrimonio')
    - Contains the year
    - Ends with .pdf
    """
    ...
    for filename in os.listdir(directory):
        ...
        if (person_name_lower in filename_lower and
            doc_type_lower in filename_lower and
            year_str in filename_lower):
...
def main():
    ...
    for year_dir in sorted(os.listdir(base_path)):
        ...
        for person_dir in sorted(os.listdir(year_path)):
            ...
            # Define document types to search for
            renta_doc_type = "Just. presentación Renta"
            patrimonio_doc_type = "Just. presentación I. Patrimonio"
            
            # Try to find Renta PDF
            renta_pdf = find_matching_file(recibido_path, person_dir, year, renta_doc_type)
            # Fallback: try without "I." in patrimonio
            if not renta_pdf:
                renta_pdf = find_matching_file(recibido_path, person_dir, year, "Just. presentación Renta")
            
            # Try to find Patrimonio PDF
            patrimonio_pdf = find_matching_file(recibido_path, person_dir, year, patrimonio_doc_type)
            # Fallback: try variations
            if not patrimonio_pdf:
                patrimonio_pdf = find_matching_file(recibido_path, person_dir, year, "Just. presentación Patrimonio")
            
            # Extract data from Renta PDF
            if renta_pdf:
                print(f"  Procesando Renta: {os.path.basename(renta_pdf)}")
            ...
            if patrimonio_pdf:
                print(f"  Procesando Patrimonio: {os.path.basename(patrimonio_pdf)}")
